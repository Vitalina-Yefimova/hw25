...
function setUserInfoCookie(key, value) {
  const date = new Date()
  date.setTime(date.getTime() + (10 * 1000)) // date.getTime() возвращает текущее время в миллисекундах
  const expires = "; expires=" + date.toUTCString() 
  
  document.cookie = key + "=" + (value || "") + expires + "; path=/" // path=/ - означает, что cookie доступно для всех страниц веб-сайта

  console.log(`Cookie "${key}=${value}" установлено на 10 секунд.`)
}
